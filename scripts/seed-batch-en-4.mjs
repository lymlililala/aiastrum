// ─── 第四批英文文章入库（24 篇：数字学 9 + 占星 7 + 解梦 8）──────────────────────
// 用法：set -a; source .env; set +a; node scripts/seed-batch-en-4.mjs
// 前置：lang 列已存在。幂等：按 slug upsert。全部为新 slug（已查无冲突）。
import { createClient } from "@supabase/supabase-js";
import { POSTS_1 } from "./en4-fill-1.mjs";
import { POSTS_2 } from "./en4-fill-2.mjs";
import { POSTS_3 } from "./en4-fill-3.mjs";
import { POSTS_4 } from "./en4-fill-4.mjs";
import { POSTS_5 } from "./en4-fill-5.mjs";
import { POSTS_6 } from "./en4-fill-6.mjs";
import { POSTS_7 } from "./en4-fill-7.mjs";
import { POSTS_8 } from "./en4-fill-8.mjs";

const SECRET = process.env.SUPABASE_SECRET_KEY;
if (!SECRET) {
  console.error("缺少 SUPABASE_SECRET_KEY。用法：set -a; source .env; set +a; node scripts/seed-batch-en-4.mjs");
  process.exit(1);
}
...
for (const p of posts) {
  ...
  if (/&amp;/.test(p.title) || /&amp;/.test(p.title_en)) { console.error(`❌ ${p.slug} title 含 &amp;（应裸 &）`); process.exit(1); }
