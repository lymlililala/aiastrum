// GSC 0718 批量文章优化生成器
// 用法：
//   node --env-file=.env scripts/gen-gsc-0718.mjs --batch=1              # 生成第1批草稿
//   node --env-file=.env scripts/gen-gsc-0718.mjs --batch=1 --publish    # 发布第1批
import { createClient } from "@supabase/supabase-js";
import { mkdirSync, writeFileSync, readFileSync, existsSync } from "node:fs";
import { join } from "node:path";
import { SPECS, LINK_POOL } from "./gsc-0718-specs.mjs";

const SUPABASE_URL = "https://tixgzezefjjsyuzgdhcd.supabase.co";
const DEEPSEEK_KEY = process.env.DEEPSEEK_API_KEY;
const SUPABASE_KEY = process.env.SUPABASE_SECRET_KEY;
const OUT_DIR = new URL("./out/gsc-0718/", import.meta.url).pathname;
...
async function generate() {
  ...
  const specs = SPECS.filter((s) => s.batch === BATCH);
