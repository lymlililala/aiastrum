...
//   默认 → 本地 data/sources.json（零 DB 依赖，开箱即用）。
//   WX_SOURCES_DB=1 且有 SUPABASE_SECRET_KEY → Supabase 表 mysticai_wx_sources（跨运行保留）。
// 接口签名：existingSns() / upsertSources(rows) / fetchSources(opts)。
//
// 启用 DB 持久化需先建表（在 Supabase SQL editor 跑）：
//   create table if not exists mysticai_wx_sources (
//     sn text primary key, account text, wxid text, title text, digest text default '',
//     content_url text, published_at timestamptz, body_text text default '',
...
const TABLE = 'mysticai_wx_sources'
// 默认本地 JSON；显式 WX_SOURCES_DB=1 才走 Supabase（避免强依赖建表）
const USE_DB = process.env.WX_SOURCES_DB === '1' && hasSupabase()
