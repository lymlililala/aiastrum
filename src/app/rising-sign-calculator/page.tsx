...
import { RisingSignInput } from "./components/RisingSignInput";
import { RisingSignResultPanel } from "./components/RisingSignResult";
import { useLocale } from "~/lib/useLocale";
import { LangSwitcher } from "../components/LangSwitcher";
import { T, type Lang } from "./rising-i18n";
import { calcRisingSign, type RisingSignResult } from "./rising-engine";
...
export default function RisingSignCalculatorPage() {
  ...
  const handleCalculate = (birthDate: string, birthTime: string, unknownTime: boolean, city: CityData) => {
    setIsLoading(true);
    // 模拟计算动画（给用户仪式感）
    setTimeout(() => {
      const r = calcRisingSign({ birthDate, birthTime, unknownTime, city, lang });
      setResult(r);
      setStep("result");
      setIsLoading(false);
            ...
            <ol style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 10 }}>
              {t.howToSteps.map((s, i) => (
                <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  ...
                  <span style={{ fontSize: "0.83rem", lineHeight: 1.7, color: "rgba(220,205,175,0.78)" }}>
                    {s}
                  </span>
                </li>
              ))}
