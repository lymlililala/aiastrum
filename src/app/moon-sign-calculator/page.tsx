...
import { MoonSignInput } from "./components/MoonSignInput";
import { MoonSignResultPanel } from "./components/MoonSignResult";
import { useLocale } from "~/lib/useLocale";
import { LangSwitcher } from "../components/LangSwitcher";
import { T, type Lang } from "./moon-i18n";
import { calcMoonSign, type MoonSignResult } from "./moon-engine";
...
export default function MoonSignCalculatorPage() {
  ...
  const handleCalculate = (birthDate: string, birthTime: string, unknownTime: boolean, city: CityData) => {
    ...
    // 模拟计算动画（给用户仪式感）
    setTimeout(() => {
      const r = calcMoonSign({ birthDate, birthTime, unknownTime, city, lang });
      setResult(r);
      setStep("result");
      setIsLoading(false);
